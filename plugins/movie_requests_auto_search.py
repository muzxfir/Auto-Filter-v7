...
async def request_admin_action(client, query):
    ...
    if status == "approved":
        ...
        search_title = request["title"].strip()
        callback_value = f"reqsearch#{search_title}"
        if len(callback_value.encode("utf-8")) > 64:
            callback_value = f"reqsearch#{search_title[:45]}"

        markup = InlineKeyboardMarkup(
            [[InlineKeyboardButton("🔍 Search Now", callback_data=callback_value)]]
        )
        label = "✅ APPROVED"
    else:
        user_text = (
