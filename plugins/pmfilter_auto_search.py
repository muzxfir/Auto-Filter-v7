...
@Client.on_callback_query(filters.regex(r"^reqsearch#"))
async def requested_movie_search(client: Client, query: CallbackQuery):
    """Search an approved request immediately inside the user's bot chat."""
    try:
        _, search = query.data.split("#", 1)
        search = search.strip()
        if not search:
            return await query.answer("Movie name is missing.", show_alert=True)
        ...
        if not files:
            return await query.answer(
                "⚠️ The movie is not indexed yet. Please try again later.",
                show_alert=True,
            )

        key = f"{query.message.chat.id}-{query.message.id}-request"
        FRESH[key] = search
        BUTTONS[key] = search
        temp.GETALL[key] = files
        temp.SHORT[query.from_user.id] = query.message.chat.id
        ...
        if offset:
            buttons.append(
                [
                    ...
                    InlineKeyboardButton(
                        f"1/{math.ceil(total_results / (10 if settings.get('max_btn', True) else int(MAX_B_TN)))}",
                        callback_data="pages",
                    ),
                    InlineKeyboardButton(
                        "Next ⏭️",
        ...
        await query.message.reply_text(
            f"🎬 <b>Search results for:</b> <code>{search}</code>\n"
            f"📂 <b>Files found:</b> {total_results}",
            reply_markup=InlineKeyboardMarkup(buttons),
            disable_web_page_preview=True,
        )
    except Exception as exc:
        logger.exception("Requested movie auto-search failed: %s", exc)
        await query.answer("❌ Search failed. Please try again.", show_alert=True)
